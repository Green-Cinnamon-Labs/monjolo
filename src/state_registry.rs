...
StateRegistry (ver docs/issue55_opcua_refactor/plan_refactor.md, seções 1.3, 6 e 7). Guarda dois
mundos, sempre distintos: CurrentState (`current_state`) — o estado real, confirmado, persistido.
Compartilhável entre threads (Art. 1.3 §1º do plano legislativo): é o "último estado físico
confirmado" — a Thread do Adaptador (e, no futuro, um Controlador) lê direto daqui, nunca de
EvaluationState. EvaluationState (`evaluation_state`) — a cópia de trabalho onde todo Proxy
lê/escreve durante uma rodada de avaliação. Pode conter valores "hipotéticos" (chute intermediário
de um solver iterativo) até alguém decidir que aquela rodada está ok. Thread-local — só a Thread da
planta toca, sem lock, sem sincronização: `Rc<RefCell<Vec<Cell<f64>>>>`.
...
/** Handle resolvido-uma-vez sobre `CurrentState` — a contraparte de `Proxy` só-leitura.
Estruturalmente parecido (buffer + índice), mas um tipo à parte de propósito: `Proxy` pode endereçar
`EvaluationState`, que pode conter valor hipotético de um solver iterativo em andamento (seção 7.2
do plano); `ReadProxy` só existe sobre `CurrentState`, sempre o último valor confirmado. Misturar os
dois tipos não compila — é assim que essa garantia vira uma propriedade do tipo, não uma regra de
disciplina de quem usa.

Diferente de `Proxy`, nasce já resolvido: `ReadProxy` só é criado depois que
`StateRegistry::resolve()` já rodou (seção 6.3) e a chave, portanto, já existe — não há segunda fase
de resolução, e não existe `set()`: quem lê `CurrentState` nunca deveria escrever nele por fora do
`commit()` do `StateRegistry`.

`buffer: Arc<RwLock<...>>`, não `Rc<RefCell<...>>`: ao contrário de `Proxy` (thread-local, sobre
`EvaluationState`), `ReadProxy` precisa atravessar thread — é o que `Sensor` usa (Art. 3.6.2/11.9),
e `Sensor` pode ser compartilhado (`Arc<Sensor>`) entre a Thread da planta, a Thread do Adaptador e
um futuro Controlador.
*/
#[derive(Clone)]
...
pub struct StateRegistry {
    ...
    dessincronizados. É sobre este buffer que `ReadProxy` resolve, uma vez, a posição que vai ler
    pra sempre. Sem nome embutido — nome é só em `index`; ver `snapshot()` pra reconstrução nomeada
    sob demanda.
    */
...
impl StateRegistry {
    ...
    */
    ...
    /** Roda uma única vez, depois que todo mundo já se inscreveu. Resolve cada input pendente
    contra a posição já conhecida (de quem ofereceu aquele nome). Se algum input não tiver provedor,
    é erro — o resto pode ter ficado parcialmente resolvido, então não adianta continuar rodando a
    simulação depois disso falhar.
    */
    ...
    deve usar isso no caminho quente — ver `read_proxy()`. None se a chave não existe ou se nenhum
    commit() rodou ainda.
    */
    ...
    /** Resolve uma chave, uma vez, contra `CurrentState` e devolve um `ReadProxy`. Só deve ser
    chamado depois que todo `DynamicModel` já se inscreveu (`subscribe()`) e `resolve()` geral já
    rodou — a chave precisa já existir em `index`; não há segunda fase de resolução como em `Proxy`.
    None se a chave não existir nesse momento (erro de configuração: sensor apontando pra algo que
    nenhum componente oferece).
    */

...
impl Simulation {
    ...
    */
    ...
    `StateRegistry` já criado nesse contexto. Ex.:
    `simulation.set_model(TennesseeEastmanModel::new)`.

    Também captura `model.state_keys()` — o que o próprio modelo declara como integrável
    (`DynamicModel`, default vazio) — enquanto o tipo ainda é `M` concreto, antes de virar `Box<dyn
    DynamicModel>` (que já não permite mais chamar métodos além do trait).
    */
    ...
    /** Chamada terminal — consome a `Simulation` (builder) e sobe a "Thread da planta" (só se
    `set_model()` foi chamado; devolve `Err` sem subir thread nenhuma caso contrário).
    ...
    */
    ...
    /** Sobe a "Thread da planta": cria `StateRegistry`, o modelo (nada disso existe antes desse
    ...
    O corpo inteiro roda dentro de `catch_unwind` — um pânico aqui (seja na inscrição inicial, seja
    em qualquer tick depois) nunca escapa da thread: vira um `ServiceEvent::Panicked` mandado pro
    canal de lifecycle.
    */
